...
<tr class="woocommerce-shipping-totals shipping">
	<th style="text-transform: none;">Shipping</th>
	<!-- <td style="padding-bottom: 30px;"><?php echo wp_kses_post( $package_name ); ?></td> -->
				...
				<?php foreach ( $available_methods as $method ) : ?>
					<!-- <li> -->
						...
						if ( 1 < count( $available_methods ) ) {
							printf( '<input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method" %4$s />', $index, esc_attr( sanitize_title( $method->id ) ), esc_attr( $method->id ), checked( $method->id, $chosen_method, false ) ); // WPCS: XSS ok.
						} else {
