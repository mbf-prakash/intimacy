...
<div class="row formWrap main-content">
					<div class="col-3 myaccount-tab">
						<div class="myaccount-sidetab" >
							 <ul>
			                <li  value="#details">My details</li>
			                <li value="#order">My order</li>
			                <li class="active" value="#vieworder">View order</li>
...
<?php if ( $post = get_page_by_path( 'order-history', OBJECT, 'page' ) ){
   ...
   <?php } ?>
						...
						<div class="myaccount-subform active" id="vieworder">

<div class="accord">View order</div>
...
<div class="accountorder_intro">
	<p><?php
	/* translators: 1: order number 2: order date 3: order status */
	printf(
		__( 'This order was placed on %2$s and was delivered to the following address:', 'woocommerce' ),
		'<strong>' . $order->get_order_number() . '</strong>',
		'<strong>' . $order->get_date_created()->format ('d M, Y') . '</strong>',
		'<strong>' . wc_get_order_status_name( $order->get_status() ) . '</strong>'
...
<div class="accountorder_intro">
	...
	/* translators: 1: order number 2: order date 3: order status */
	printf(
		__( 'This order was placed on %2$s and is currently being processed.', 'woocommerce' ),
		'<strong>' . $order->get_order_number() . '</strong>',
		'<strong>' . $order->get_date_created()->format ('d M, Y') . '</strong>',
		'<strong>' . wc_get_order_status_name( $order->get_status() ) . '</strong>'
